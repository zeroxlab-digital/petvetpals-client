...
const PetProfiles = () => {
    ...
    return (
        <div className='grid grid-cols-3 gap-10 '>
            {profiles.map(({ _id, type, name, age, breed, gender, weight, createdAt, updatedAt }) => (
                <div key={_id} className='border rounded-md bg-white '>
                    <Image src="/images/vet.png" alt="pet's-img" width={200} height={200} className='object-cover w-full h-56 rounded-t-md rounded-b-3xl' />
                    <div className='p-3 mt-3'>
                        <ul className='text-gray-600 flex flex-col gap-3 mb-7'>
                            ...
                            <li className='flex items-center justify-between text-sm'><span>CreatedAt</span>{createdAt}</li>
                            <li className='flex items-center justify-between text-sm'><span>UpdatedAt</span>{updatedAt}</li> */}
                        </ul>
                        <Button variant={"primary"} classNames={"!w-full !text-base !font-normal"}>Update Profile</Button>
                    </div>
                </div>
            ))}
            <div className=' border-dashed border-2 rounded-md flex items-center justify-center'>
                <button className='text-gray-500 text-lg'><span className='text-xl'>+</span> Add new profile</button>
            </div>
        </div>
    );
};
