            ...
            {filtered_appointments.length > 0 ? (
                <div className='space-y-4'>
                    {filtered_appointments.map((appointment, index) => (
                        <div key={index} className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 items-center gap-5 border rounded-md p-5'>
                            ...
                            <div className='flex flex-wrap gap-4 lg:col-span-3 justify-end'>
                                {active_status_tab === 'confirmed' ? (
                                    <>
                                        <button className='bg-white px-5 py-3 rounded-lg border text-gray-900 text-sm flex items-center gap-2 max-lg:w-full max-lg:justify-center'>Edit <HiChevronDown /></button>
                                        <button className='bg-primary px-5 py-3 rounded-lg text-white text-sm flex items-center gap-2 max-lg:w-full max-lg:justify-center'>Join Now <HiVideoCamera /></button>
                                    </>
                                ) : active_status_tab === 'pending' ? (
                                    <>
                                        <button onClick={() => handleConfirmBooking(appointment)} className='bg-primary px-5 py-3 rounded-lg text-white text-sm flex items-center gap-2 max-lg:w-4/5 max-lg:justify-center'>Confirm <HiArrowRight /></button>
                                        <button onClick={() => handleAppointmentDlt(appointment._id)} className='bg-white px-3 py-3 rounded-lg text-red-400 border text-sm flex items-center  '><HiOutlineTrash /></button>
