...
const ScheduleMedication = ({ onClose, ongoingMedications, petId, schedule = null, refetch }) => {
    ...
    useEffect(() => {
        ...
        console.log(med)
        if (med) {
            setFormData(prev => ({ ...prev, starting_date: med.start_date || '', end_date: med.end_date || '' }));
        }
    }, [formData.medId, ongoingMedications]);
                ...
                <div className='grid grid-cols-2 gap-3 max-md:grid-cols-1 my-3'>
                    <div>
                        ...
                        <Input
                            id="startDate"
                            type="date"
                            value={formData.starting_date ? formData.starting_date.split('T')[0] : ''}
                        ...
                        <div key={index} className="grid grid-cols-2 gap-3 items-end">
                            ...
                            <div>
                                ...
                                <SelectOptions
                                    ...
                                    placeholder={`
                                        ${
                                            reminder.remind_before == '60' ? '1 hour before' : reminder.remind_before !== '0' ? `${reminder.remind_before} minutes before` : 'At dose time'
                                        }
                                    `}
                                    onChange={(e) => {
                                        const updated = [...formData.reminder_times];
                                        updated[index].remind_before = e.target.value;
                                        handleChange('reminder_times', updated);
                                    }}
                                />
                            </div>
