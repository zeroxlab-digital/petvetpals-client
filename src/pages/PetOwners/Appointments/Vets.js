        ...
        <div className='flex flex-col gap-5'>
            {
                vets?.map(({ _id, avator, name, title, works_at, years_of_experiences, specialities, visit_fee_usd, visit_fee_bdt }) => (
                    <Link href={{
                        ...
                        <div className='flex sm:items-start gap-5 max-sm:gap-3'>
                            ...
                            <div>
                                ...
                                <div className='flex gap-1 flex-wrap sm:hidden'>{specialities.slice(0, 2).map((speciality, index) => <p key={index} className='text-xs bg-primary p-1 text-white rounded'>{speciality}</p>)}</div>
                                <div className='flex gap-1 flex-wrap max-sm:hidden'>{specialities.map((speciality, index) => <p key={index} className='text-xs bg-primary p-1 text-white rounded'>{speciality}</p>)}</div>
                            </div>
                        </div>
